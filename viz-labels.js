...
var links = [
    ...
    {"source": 1, "target": 6, "rel": "critical"},
...
// add the text
    ...
    var anchorNode = rootg.selectAll("g.anchorNode").data(force2.nodes()).enter().append("svg:g").attr("class", "anchorNode");
...
function tick() {
    ...
    $('#search').on('keyup', function(){
      //TODO: update to use enter to lock in search as currently searching for project 3
    ...
    if(nodeID > ""){
        $('#' + nodeID + ', [data-from="' + nodeID + '"], [data-to="' + nodeID + '"]').each(function(){
            ...
            if($(this).is('[data-to="' + nodeID + '"]')){
              $('#' + $(this).attr('data-from')).removeClass('dimmed');
            }
